...
import { NetworkContent, useContentGenres } from '@/hooks/useCinema';
import { useUpdateContent } from '@/hooks/useNetworkContentManage';
import { Film, Upload, Link as LinkIcon, X, Plus, Loader2 } from 'lucide-react';
...
export const EditContentDialog = ({ open, onOpenChange, content }: EditContentDialogProps) => {
  ...
  useEffect(() => {
    if (content) {
      ...
      setRuntimeMinutes(content.runtime_minutes?.toString() || '');
      setReleaseDate(content.release_date || '');
      setExternalVideoUrl(content.external_video_url || '');
      setGenreTags(content.genre_tags || []);
      setCastMembers(content.cast_members || []);
      setIsFeatured(content.is_featured);
      setCoverPreview(content.cover_art_url);
      setCoverFile(null);
    }
  }, [content]);
  ...
  const handleCoverChange = (file: File | null) => {
    ...
    if (file) {
      ...
      setCoverPreview(content?.cover_art_url || null);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let coverArtUrl = content.cover_art_url;

      // Upload new cover if selected
      if (coverFile) {
        const uploadedUrl = await uploadFile(coverFile, 'content-covers');
        if (uploadedUrl) {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex gap-3">
            ...
            <Button
              type="submit"
              className="flex-1"
              disabled={uploading || updateContent.isPending}
            >
              {uploading || updateContent.isPending ? (
                <>
