...
export const MusicProfileBlock = ({ userId, onConnectClick }: MusicProfileBlockProps) => {
  ...
  // Get the featured album image (latest release or first album)
  const featuredImage = musicProfile.latest_release?.image_url || 
                        musicProfile.albums[0]?.image_url || 
    ...
    >
      ...
      {featuredImage && (
        <div className="relative h-32 sm:h-40">
          <img
            src={featuredImage}
            alt={musicProfile.latest_release?.name || "Album cover"}
            className="w-full h-full object-cover"
          />
          <div className="absolute inset-0 bg-gradient-to-t from-background via-background/60 to-transparent" />
          
          {/* Platform Links on banner */}
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {/* Platform Links (only show if no banner) */}
          {!featuredImage && (
