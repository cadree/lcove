...
const HomeTopBar = ({
  ...
  return <motion.header initial={{
    ...
    duration: 0.5,
    ease: [0.16, 1, 0.3, 1]
  }} className="sticky top-0 z-40 px-4 pt-4" style={{
    paddingTop: 'max(env(safe-area-inset-top, 16px), 16px)'
      ...
      <div className="glass-strong rounded-2xl px-4 py-3 shadow-elevated flex items-center justify-between">
        ...
        {/* Left - Menu */}
        <Button variant="ghost" size="icon" className="w-10 h-10 rounded-xl text-muted-foreground hover:text-foreground hover:bg-accent/40" onClick={onMenuClick}>
        ...
        <div className="flex items-center gap-2">
          {!isOnMembership && <Button variant="ghost" size="sm" className="h-9 px-3 rounded-xl text-primary hover:text-primary hover:bg-primary/10 gap-1.5" onClick={() => navigate("/membership")}>
              <Heart className="w-4 h-4" />
              <span className="text-xs font-medium hidden sm:inline">Contribute</span>
            </Button>}
          {user && <EnergyIndicator />}
          <Link to="/notifications">
            <Button variant="ghost" size="icon" className="w-10 h-10 rounded-xl text-muted-foreground hover:text-foreground hover:bg-accent/40">
