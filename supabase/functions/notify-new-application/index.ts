...
interface NotifyNewApplicationRequest {
  project_id: string;
...
const getEmailTemplate = (projectTitle: string, roleTitle: string, applicantName: string) => {
  return `
    <!DOCTYPE html>
    <html>
      <head>
                ...
                <tr>
                  <td style="padding: 40px; text-align: center;">
                    ...
                    <p style="color: #a0a0a0; font-size: 16px; line-height: 1.6; margin: 0 0 20px;">
                      <strong style="color: #E91E63;">${applicantName}</strong> has applied for the role of <strong style="color: #ffffff;">${roleTitle}</strong> on your project:
                    </p>
...
serve(async (req) => {
  ...
  try {
    ...
    const { project_id, project_creator_id, project_title, role_title, applicant_name }: NotifyNewApplicationRequest = await req.json();

    console.log("Notifying project owner of new application:", project_creator_id, "for project:", project_title);

    // Get user data
    const { data: userData, error: userError } = await supabase.auth.admin.getUserById(project_creator_id);
    ...
    if (prefs?.application_updates_enabled === false) {
      console.log("Application update notifications disabled for user:", project_creator_id);
      return new Response(
    ...
    const notificationTitle = "New Project Application";
    const notificationBody = `${applicant_name} applied for "${role_title}" on your project "${project_title}"`;
    ...
    const { error: notifError } = await supabase
      ...
      .insert({
        ...
        data: { project_id, role_title, applicant_name }
      });
      ...
      try {
        const emailResponse = await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            ...
            html: getEmailTemplate(project_title, role_title, applicant_name),
          }),
        });
    ...
    console.error("Error in notify-new-application:", errorMessage);
    return new Response(
