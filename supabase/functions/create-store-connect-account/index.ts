...
const logStep = (step: string, details?: unknown) => {
  ...
  console.log(`[CREATE-STORE-CONNECT] ${step}${detailsStr}`);
};
...
serve(async (req) => {
  ...
  try {
    ...
    logStep("Store verified", { storeId, storeName: store.name });
    ...
    const accountLink = await stripe.accountLinks.create({
      account: accountId,
      refresh_url: `${origin}/store?connect_refresh=true`,
      return_url: `${origin}/store?connect_success=true`,
