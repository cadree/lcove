...
-- Accept a custom role proposal atomically
...
DECLARE
  ...
  v_new_role_id UUID;
BEGIN
  SELECT * INTO v_application FROM project_applications WHERE id = p_application_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'Application not found'; END IF;
  IF v_application.status != 'pending' THEN RAISE EXCEPTION 'Application is not pending'; END IF;

  SELECT * INTO v_project FROM projects WHERE id = v_application.project_id;
  IF v_project.creator_id != p_reviewer_id THEN RAISE EXCEPTION 'Only the project creator can accept applications'; END IF;
  ...
  UPDATE project_applications SET role_id = v_new_role_id WHERE id = p_application_id;
  UPDATE project_applications SET status = 'accepted', reviewed_by = p_reviewer_id, reviewed_at = now() WHERE id = p_application_id;
